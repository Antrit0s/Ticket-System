...
// The logged-in user's own tickets, newest first by default, with a text search
...
export default function UserTicketsPage() {
  ...
  // Sort client-side (json-server beta has no _sort) and filter by title/ID.
  const visibleTickets = useMemo(() => {
    ...
    return sortedTickets.filter(
      (ticket) =>
        !searchQuery || ticket.title.toLowerCase().includes(searchQuery) || ticket.id.toLowerCase().includes(searchQuery),
    );
  ...
  return (
    <Box>
      ...
      >
        ...
        <Button variant="contained" onClick={() => navigate("/dashboard/tickets/new")}>
          New ticket
        </Button>
      </Box>
